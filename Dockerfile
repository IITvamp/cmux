...
RUN --mount=type=cache,target=/usr/local/cargo/registry \
  ...
  cargo install --path crates/vnc-ws-proxy --target x86_64-unknown-linux-gnu --locked --force; \
  else \
  ...
  cargo install --path crates/vnc-ws-proxy --locked --force; \
  fi
...
COPY --from=rust-builder /usr/local/cargo/bin/vnc-ws-proxy /usr/local/bin/vnc-ws-proxy

# Configure envctl/envd runtime defaults
RUN chmod +x /usr/local/bin/envctl /usr/local/bin/envd /usr/local/bin/cmux-proxy /usr/local/bin/vnc-ws-proxy && \
  envctl --version && \
