...
async function withRetry<T>(
  ...
  throw (
    lastError ||
    new Error(`Failed after ${MAX_RETRIES} attempts: ${description}`)
  );
}
...
export async function markTaskRunComplete(
  ...
  return withRetry(async () => {
    ...
    const result = await convexClient.mutation(
      api.taskRuns.workerMarkComplete,
      {
...
export async function markTaskRunFailed(
  ...
  return withRetry(async () => {
    log("INFO", `Marking taskRun ${taskRunId} as failed`, {
      errorMessage,
      exitCode,
    });
 ...
 */
export async function checkAllTaskRunsComplete(taskId: string): Promise<{
  allComplete: boolean;
  hasRuns: boolean;
  ...
  return withRetry(async () => {
    const result = await convexClient.query(
      api.taskRuns.checkAllTaskRunsComplete,
      {
