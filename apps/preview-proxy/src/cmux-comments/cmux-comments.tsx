  ...
  useEffect(() => {
    const updatePosition = () => {
      try {
        ...
        // First try the selector as-is (for new comments with escaped selectors)
        try {
          el = document.querySelector(comment.nodeId) as HTMLElement;
        } catch (_e) {
          // If that fails, try escaping the selector (for old comments)
          const escapedSelector = comment.nodeId.replace(/([:])/g, '\\$1');
          try {
            el = document.querySelector(escapedSelector) as HTMLElement;
          } catch (_e2) {
            // If both fail, log and give up
            console.warn(`Could not find element with selector: ${comment.nodeId}`);
          }
        }
        ...
        console.error("Failed to find element for comment:", e, "Selector:", comment.nodeId);
        setPosition(null);
      }
    };
...
function CmuxCommentsWidget() {
  ...
  useEffect(() => {
    ...
    const handleClick = async (e: MouseEvent) => {
      ...
      const getSelector = (el: HTMLElement): string => {
        ...
        while (current && current !== document.body) {
          ...
          if (current.id) {
            ...
            // Filter out hover classes and escape special characters in class names
            const classes = Array.from(current.classList)
              .filter(c => !c.startsWith("hover"))
              .map(c => {
                // Escape special characters in class names (for Tailwind classes like md:grid-cols-2)
      ...
      const nodeId = getSelector(element);
