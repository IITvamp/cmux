...
Please address the issue mentioned in the comment above.`;
    ...
    // Create a draft PR for a run using Octokit only; no local git operations
    socket.on("github-create-draft-pr", async (data, callback) => {
      try {
        const { taskRunId } = GitHubCreateDraftPrSchema.parse(data);

        // Load run and task directly; avoid worktree/git side effects
        const run = await convex.query(api.taskRuns.get, {
          id: taskRunId as Id<"taskRuns">,
        });
        if (!run) {
        ...
        if (!owner || !repo) {
          callback({ success: false, error: "Unknown repository for task" });
          return;
        }

        const branchName = run.newBranch || `cmux-run-${String(run._id).slice(-8)}`;
        // Determine base branch: prefer task.baseBranch, else fetch repo default
        let baseBranch = task.baseBranch || "";
        if (!baseBranch) {
          baseBranch = (await fetchDefaultBranch(githubToken, owner, repo)) || "main";
        }

        // Use latest commit message for PR title/body
        const latestMsg = await fetchLatestCommitMessage(
          githubToken,
          owner,
          repo,
          branchName
        );
        const fallbackTitle = task.pullRequestTitle || task.text || "cmux changes";
        const fullMessage = (latestMsg && latestMsg.trim()) || fallbackTitle;
        const firstLine = fullMessage.split("\n")[0] || fallbackTitle;
        const truncatedTitle =
          firstLine.length > 72 ? `${firstLine.slice(0, 69)}...` : firstLine;
        // Body: use entire commit message (including first line) for rich detail
        const body = fullMessage;

        // If a PR already exists for this head, return it; otherwise create a draft PR
        const existing = await fetchPrByHead(
          githubToken,
          owner,
          repo,
          owner,
        ...
        if (existing) {
          ...
          try {
            ...
            const msg = e instanceof Error ? e.message : String(e);
            callback({ success: false, error: `Failed to create draft PR: ${msg}` });
            return;
          }
        }
    ...
    // Open PR: create a non-draft PR if missing, or mark draft PR as ready (Octokit-only)
    socket.on("github-open-pr", async (data, callback) => {
      try {
        const { taskRunId } = GitHubCreateDraftPrSchema.parse(data);

        // Load run/task directly to avoid local git activity
        ...
        const branchName = run.newBranch || `cmux-run-${String(run._id).slice(-8)}`;
        // Determine base branch: prefer task.baseBranch; else fetch default from GitHub
        let baseBranch = task.baseBranch || "";
        if (!baseBranch) {
          baseBranch = (await fetchDefaultBranch(githubToken, owner, repo)) || "main";
        }

        // Derive title/body from latest commit message on the branch
        const latestMsg = await fetchLatestCommitMessage(
          githubToken,
          owner,
          repo,
