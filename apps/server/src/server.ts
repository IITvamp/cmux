...
Please address the issue mentioned in the comment above.`;
    ...
    // Create a draft PR for a crowned run: assume branch is already up to date and pushed
    socket.on("github-create-draft-pr", async (data, callback) => {
      try {
        ...
        // Ensure on branch, commit (no-op safe), and create draft PR using local filesystem
    ...
    // Open PR: create a non-draft PR if missing, or mark draft PR as ready (no pull/push here)
    socket.on("github-open-pr", async (data, callback) => {
      try {
        ...
        // Stage/commit (no-op safe). Do not pull/push here; assume auto-commit/push is already done.
        try {
          await execAsync("git add -A", { cwd, env: { ...process.env } });
          await execAsync(
            `git commit -m ${JSON.stringify(commitMessage)} || echo 'No changes to commit'`,
