...
import { containerMappings, type ContainerMapping, DockerVSCodeInstance } from "./vscode/DockerVSCodeInstance.js";
import { VSCodeInstance } from "./vscode/VSCodeInstance.js";

export function createProxyApp(vscodeInstances: Map<string, VSCodeInstance>): express.Application {
  ...
  app.use(async (req: express.Request, res: express.Response, next: express.NextFunction) => {
    ...
    if (!containerName || !targetPort) {
      return res.status(400).send('Invalid subdomain format. Expected: containerName.port.localhost:3001');
    }
    ...
    if (!mapping) {
      ...
      for (const [instanceId, instance] of Array.from(vscodeInstances.entries())) {
        if (instance instanceof DockerVSCodeInstance) {
          // Check if the instance ID or any part matches the container name
          if (instanceId.includes(containerName) || containerName.includes(instanceId)) {
      ...
      if (!matchedInstance) {
        return res.status(404).send(`No VSCode instance found for container: ${containerName}`);
      }
      ...
      return res.status(400).send('Only Docker VSCode instances are supported for proxy');
    }
    ...
    if (mapping.status === 'stopped') {
      ...
      return res.status(503).send(`Container ${containerName} is stopped and cannot be restarted`);
    }
    ...
    if (!actualPort) {
      return res.status(400).send(`Port ${targetPort} not found for container ${containerName}`);
    }
    ...
    const proxy = createProxyMiddleware({
      ...
      on: {
        error: (err: Error, req: express.Request, res: express.Response | any) => {
          console.error(`Proxy error for ${containerName}:${actualPort}:`, err.message);
        ...
        proxyReq: (proxyReq: any, req: express.Request, res: express.Response) => {
          // Log proxy requests
          console.log(`Proxying ${req.method} ${req.url} -> localhost:${actualPort}${req.url}`);
        },
      },
    });
