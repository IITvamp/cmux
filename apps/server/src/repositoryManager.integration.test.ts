...
describe("RepositoryManager - Integration Tests", () => {
  ...
  beforeAll(async () => {
    ...
    await execAsync(`git remote add origin ${bareRepoPath}`, { cwd: testRepoPath });
  ...
  it("should detect if worktree exists", async () => {
    ...
    // Create worktree
    await execAsync(`git worktree add -b ${branchName} "${worktreePath}" main`, { cwd: testRepoPath });
  ...
  it("should execute git commands", async () => {
    const { stdout } = await repoManager.executeGitCommand("git status --short", { cwd: testRepoPath });
  ...
  it("should handle branch already used by another worktree", async () => {
    ...
    await repoManager.createWorktree(testRepoPath, oldWorktreePath, branchName, "main");
    
    // Verify old worktree exists
    let exists = await fs.access(oldWorktreePath).then(() => true).catch(() => false);
    expect(exists).toBe(true);
    
    // Try to create new worktree with same branch - should remove old one
    await repoManager.createWorktree(testRepoPath, newWorktreePath, branchName, "main");
    
    // Verify new worktree exists
    exists = await fs.access(newWorktreePath).then(() => true).catch(() => false);
    expect(exists).toBe(true);
    
    // Verify old worktree was removed from git
    const oldWorktreeRegistered = await repoManager.worktreeExists(testRepoPath, oldWorktreePath);
    expect(oldWorktreeRegistered).toBe(false);
    
    // Clean up
    await execAsync(`git worktree remove "${newWorktreePath}" --force`, { cwd: testRepoPath }).catch(() => {});
  });
