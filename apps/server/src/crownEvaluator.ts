...
Completed: ${new Date().toISOString()}`;
    ...
    if (extensionPort) {
      ...
      if (extensionResult.success) {
        if (githubToken) {
          serverLogger.info(
            `[CrownEvaluator] Successfully created PR via VSCode extension`
          );
        } else {
          serverLogger.info(
            `[CrownEvaluator] Successfully pushed branch via VSCode extension`
          );
          serverLogger.info(
      ...
      serverLogger.info(
        `[CrownEvaluator] VSCode extension method failed:`,
  ...
  try {
    ...
    return new Promise((resolve) => {
      ...
      extensionSocket.on("connect", () => {
        serverLogger.info(
          `[CrownEvaluator] Connected to VSCode extension on port ${extensionPort}`
        );
        ...
        extensionSocket.emit(
          ...
          (response: any) => {
            ...
            if (githubToken) {
              extensionSocket.emit(
                ...
                (prResponse: any) => {
                  ...
                  if (prResponse.success) {
                    serverLogger.info(
                      `[CrownEvaluator] PR created successfully via VSCode extension`
                    );
                    if (prResponse.result?.stdout) {
                      serverLogger.info(
                        `[CrownEvaluator] PR URL: ${prResponse.result.stdout.trim()}`
                      );
                    }
                    resolve({ success: true });
                  } else {
                    serverLogger.error(
                      `[CrownEvaluator] PR creation failed:`,
                      prResponse.error
                    );
              ...
              extensionSocket.disconnect();
              serverLogger.info(
                `[CrownEvaluator] Branch pushed successfully via VSCode extension (PR creation skipped - no token)`
              );
              resolve({ success: true });
            }
          }
