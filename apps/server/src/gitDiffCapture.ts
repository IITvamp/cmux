...
export async function captureGitDiffViaTerminal(
  ...
  try {
    ...
    serverLogger.info(`[GitDiffCapture] ========================================`);
    serverLogger.info(`[GitDiffCapture] CAPTURING GIT DIFF VIA VSCODE TERMINAL`);
    ...
    const outputHandler = (data: any) => {
      ...
      if (data.terminalId === originalTerminalId && data.data) {
        if (data.data.includes(captureMarker)) {
          ...
          serverLogger.info(`[GitDiffCapture] Captured ${data.data.length} chars, total: ${terminalOutput.length}`);
        }
      }
    };
    ...
    try {
      ...
      await sendTerminalCommand(workerSocket, originalTerminalId, "git diff HEAD");
      ...
      serverLogger.info(`[GitDiffCapture] Raw terminal output length: ${terminalOutput.length}`);
      serverLogger.info(`[GitDiffCapture] First 200 chars of raw output: ${terminalOutput.substring(0, 200)}`);
      serverLogger.info(`[GitDiffCapture] Cleaned diff length: ${cleanedDiff.length}`);
      
      if (!cleanedDiff || cleanedDiff.length < 10) {
        serverLogger.warn(`[GitDiffCapture] No diff captured, trying alternative approach`);
        ...
        serverLogger.info(`[GitDiffCapture] Alternative approach - raw output length: ${terminalOutput.length}`);
        serverLogger.info(`[GitDiffCapture] Alternative approach - cleaned diff length: ${cleanedDiff.length}`);
      }
      
      serverLogger.info(`[GitDiffCapture] Captured diff length: ${cleanedDiff.length}`);
