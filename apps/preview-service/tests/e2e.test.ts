...
describe("Preview Service E2E Tests", () => {
  ...
  beforeAll(async () => {
    ...
    if (!baseSnapshotId) {
      ...
      console.warn(
        "   Some tests will be skipped. Run `pnpm test:snapshot` to create one.\n"
      );
    }
  });
  ...
  describe("Preview Management", () => {
    ...
    it("should set base snapshot", async () => {
      const response = await fetch(
        `${serverUrl}/api/preview/set-base-snapshot`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
    ...
    it(
      ...
      async () => {
        ...
        const createData = PreviewResponseSchema.parse(
        ...
        const statusResponse = await fetch(
          `${serverUrl}/api/preview/status/${previewId}`
        );
        const statusData = PreviewResponseSchema.parse(
        ...
        for (let i = 0; i < 3; i++) {
          try {
            const workerResponse = await fetch(
              `${statusInfo.urls.worker}/socket.io/?EIO=4&transport=polling`
            );
        ...
        if (!workerOk) {
          console.log(
            "⚠️  Worker socket.io endpoint not available, but exec functionality works"
          );
        }
        ...
        const pauseResponse = await fetch(
          `${serverUrl}/api/preview/pause/${previewId}`,
          {
            method: "POST",
          }
        );
        const pauseData = PreviewResponseSchema.parse(
        ...
        const resumeResponse = await fetch(
          `${serverUrl}/api/preview/resume/${previewId}`,
          {
            method: "POST",
          }
        );
        const resumeData = PreviewResponseSchema.parse(
        ...
        const stopResponse = await fetch(
          `${serverUrl}/api/preview/stop/${previewId}`,
          {
            method: "POST",
          }
        );
