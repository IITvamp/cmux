  ...
  const { summaryIcon, summaryText, summaryColorClass } = hasAnyRunning
    ...
    : hasAnyFailure
      ? {
          ...
          summaryText: `${failedRuns.length} ${
            failedRuns.length === 1 ? "check" : "checks"
          } failed`,
      ...
      : allPassed
        ...
        : {
            ...
            summaryText: `${sortedRuns.length} ${
              sortedRuns.length === 1 ? "check" : "checks"
            }`,
          ...
          {sortedRuns.map((run) => {
            ...
            return (
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="flex items-center justify-between gap-2 pl-8 pr-3 py-2 text-xs text-neutral-600 dark:text-neutral-300 hover:bg-neutral-100 dark:hover:bg-neutral-800/50 transition-colors"
              >
                ...
                <span className="text-[11px] text-neutral-400 dark:text-neutral-500 whitespace-nowrap">
                  {run.timestamp ? new Date(run.timestamp).toLocaleTimeString() : ""}
                </span>
              </a>
            );
          })}
