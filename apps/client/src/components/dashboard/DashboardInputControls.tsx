...
export const DashboardInputControls = memo(function DashboardInputControls({
  ...
  const handleAgentRemove = useCallback(
    (agent: string) => {
      // Remove a single occurrence of the agent
      const next = [...selectedAgents];
      const index = next.indexOf(agent);
      if (index >= 0) {
        next.splice(index, 1);
        onAgentChange(next);
      }
              ...
              return (
                ...
                >
                  ...
                  <button
                    ...
                    className="ml-1 inline-flex h-4 w-4 items-center justify-center rounded-full transition-colors hover:bg-neutral-300 dark:hover:bg-neutral-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-neutral-400/60"
                    title="Add one more"
                  >
