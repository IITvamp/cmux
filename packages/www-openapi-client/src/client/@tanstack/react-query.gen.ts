...
export const getApiIframePreflightOptions = (options: Options<GetApiIframePreflightData>) => {
    return queryOptions({
        queryFn: async ({ queryKey, signal }) => {
            const { data } = await getApiIframePreflight({
                ...options,
                ...queryKey[0],
                signal,
                throwOnError: true
        ...
        queryKey: getApiIframePreflightQueryKey(options)
    });
};
