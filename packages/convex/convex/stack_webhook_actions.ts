...
export const syncUserMembershipsFromStack = internalAction({
  args: { userId: v.string() },
  handler: async (ctx, { userId }) => {
    try {
      const user = await stackServerAppJs.getUser(userId);
      if (!user) {
        console.warn("[stack_webhook] User not found in Stack during membership sync", {
          userId,
        });
        return;
      }
      ...
      console.error("[stack_webhook] Failed to sync user memberships", {
        userId,
        error,
      });
    }
