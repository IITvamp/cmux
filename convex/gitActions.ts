...
async function getAppDataPath(): Promise<string> {
  // Get the app name from package.json or use a default
...
export const setupProjectWorkspace = action({
  args: {
    repoUrl: v.string(),
    branch: v.optional(v.string()),
  },
  handler: async (_ctx, args): Promise<WorkspaceResult> => {
    try {
      ...
      try {
        // Create a new branch based on the base branch and add worktree
        await execAsync(
