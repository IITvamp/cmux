...
# CI macOS universal binary build + sign + notarize for Electron app
# Creates a universal binary from x64 and arm64 builds
...
Builds, signs, notarizes macOS universal binary (x64 + arm64) DMG/ZIP locally.
...
Notes:
  - This script creates a universal binary combining x64 and arm64 architectures.
...
echo "==> Packaging universal binary (signed; built-in notarize disabled due to macOS 15 notarytool JSON issue)"
