...
try {
  ...
  const version = await instance.exec(["/usr/bin/gh", "--version"]);
  ...
  const scriptSmoke = ["set -euo pipefail", "echo script helper ok"].join("\n");
  const scriptRun = await instance.exec(scriptSmoke);
  if (scriptRun.exit_code !== 0) {
    throw new Error(
      `rootfs script helper failed with exit ${scriptRun.exit_code}. stderr=${scriptRun.stderr}`
    );
  }
  console.log("[verify-gh-rootfs] instance.exec script output:");
