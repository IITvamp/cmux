...
const agentFiles = ['CLAUDE.md', 'GEMINI.md', 'OPENAI.md', 'AGENTS.md'];
const rootDir = process.cwd();

function syncAgentDocs() {
  // Find the first existing agent file
...
`;
    writeFileSync(join(rootDir, 'CLAUDE.md'), defaultContent);
    sourceFile = 'CLAUDE.md';
    sourceContent = defaultContent;
  }

  console.log(`Using ${sourceFile} as source to sync other agent files...`);

  // Copy to missing files
  for (const file of agentFiles) {
    if (file !== sourceFile) {
      const filePath = join(rootDir, file);
      if (!existsSync(filePath)) {
  ...
  console.log('Agent documentation files synced successfully!');
}
