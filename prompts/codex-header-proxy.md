read apps/www/app/api/anthropic/v1/messages/route.ts and do the same for openai endpoint proxy, figure out if codex cli can inject additional headers using config files, and inject similar cmux token that we do for anthropic claude code


here's docs for codex cli config:
