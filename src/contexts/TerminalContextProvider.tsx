...
export const TerminalContextProvider: React.FC<TerminalContextProviderProps> = ({
  ...
  useEffect(() => {
    ...
    socketRef.current.on("terminal-created", ({ terminalId }: { terminalId: string }) => {
      ...
      const xterm = new XTerm({
        fontSize: 12,
        fontFamily: "Menlo, Monaco, operator mono,SFMono-Regular,Consolas,Liberation Mono,Menlo,monospace",
        theme: {
          background: "#1e1e1e",
          foreground: "#d4d4d4",
          cursor: "#aeafad",
    ...
    socketRef.current.on("terminal-output", ({ terminalId, data }: { terminalId: string; data: string }) => {
    ...
    socketRef.current.on("terminal-exit", ({ terminalId, exitCode }: { terminalId: string; exitCode: number }) => {
      const terminal = terminalsRef.current.get(terminalId);
      if (terminal) {
    ...
    socketRef.current.on("terminal-clear", ({ terminalId }: { terminalId: string }) => {
      const terminal = terminalsRef.current.get(terminalId);
      if (terminal) {
    ...
    socketRef.current.on("terminal-closed", ({ terminalId }: { terminalId: string }) => {
      const terminal = terminalsRef.current.get(terminalId);
      if (terminal) {
  ...
  const createTerminal = useCallback((_id?: string, _name?: string): string => {
    // Request server to create a new terminal
    socketRef.current?.emit("create-terminal", { cols: 80, rows: 24 });
    
    // Return a temporary ID - the actual terminal will be created when we receive terminal-created event
  ...
  const getTerminal = useCallback((id: string): TerminalInstance | undefined => {
